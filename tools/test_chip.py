...
from mmdet.datasets.pipelines.loading_chip import LoadAnnotations_chip
...
parsing_name = ['background','Hat', 'Hair', 'Glove', 'Sunglasses', 'Upper-clothes', 
...
def main():
    ...
    for epoch_num in range(12,13):
        ...
        print("Generating GT...")
        dat_list = dataset.img_infos
        get_gt_cate(dat_list, nb_class=20, ovthresh_seg=0.5)
        ...
        if eval_types == ['parsing']:
            ...
            _iou, _miou, mean_acc, pixel_acc = parsing_iou('eval_file/pred_cate/', 'eval_file/gt_cate', 20)
            ...
            all_ap_p, all_pcp, miou = eval_parsing_ap(outputs, dat_list, nb_class=20, ovthresh_seg=0.5,
            ...
            Insmetrcs = InstanceMetrics(pred_output_dir, gt_output_dir, num_classes=15, categories=DENSEPOSE_CATEGORIES)
            image_name_list = [
                x[:-4] for x in os.listdir(pred_output_dir) if x[-3:] == 'txt'
            ]
            AP_map = Insmetrcs._compute_class_ap(image_name_list, 1)
            AP_map = np.mean(AP_map)
            print('AP^r: {}'.format(
                AP_map
            ))
