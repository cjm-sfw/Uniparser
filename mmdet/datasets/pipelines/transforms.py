...
class Pad_Parsing:
    ...
    def __init__(self, size=None, size_divisor=None, pad_val=0, random_val=False):
        self.size = size
        self.size_divisor = size_divisor
        self.pad_val = pad_val
        if random_val:
            import random
            self.pad_val = random.randint(0, pad_val)
        # only one of size and size_divisor should be valid
        assert size is not None or size_divisor is not None
        assert size is None or size_divisor is None
    ...
    def _pad_parsings(self, results):
        ...
        for key in results.get('parsing_fields', []):
            padded_parsings = [
                mmcv.impad(parsing, shape=pad_shape, pad_val=self.pad_val)
                for parsing in results[key]
            ]
            if padded_parsings:
