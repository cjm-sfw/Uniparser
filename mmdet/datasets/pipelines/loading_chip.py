...
class LoadImageFromFile_chip(object):
    """
    input: result --- [{
                    'filepath' : img_add
                    'width' : image.size[1]
...
@PIPELINES.register_module
class LoadAnnotations_chip(object):
    """
    input: result --- [{
                    'filepath' : img_add
                    'width' : image.size[1]
    ...
    def __repr__(self):
        ...
        repr_str += ('(with_bbox={}, with_label={}, with_mask={}, with_mhp_parsing={}'
                     ' with_seg={})').format(self.with_bbox, self.with_label,
                                             self.with_mask, self.with_mhp_parsing, self.with_seg)
        return repr_str
