        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = mmcv.imrescale(
                    results[key], results['scale_factor'], interpolation='nearest')
            else:
    ...
    def _resize_keypoints(self, results):
        ...
        for key in results.get('keypoints_fields', []):
            if type(results[key]) is not list:
                keypoints = np.ones((1,20,3)) * -1
            else:
                keypoints = np.array(results[key])
            if len(keypoints.shape) == 3:
                keypoints[:, :, :2] = keypoints[:,:,:2]*results['scale_factor']
                keypoints[:, :, 0] = np.clip(keypoints[:, :,0], -1, img_shape[1] - 1)
                keypoints[:, :, 1] = np.clip(keypoints[:, :,1], -1, img_shape[0] - 1)
                results[key] = keypoints
            else:
                results[key] = np.ones((1,20,3)) * -1
...
@PIPELINES.register_module
class RandomFlip_Parsing(object):
    ...
    def __init__(self, flip_ratio=None, direction='horizontal', dataset='mhp'):
        self.flip_ratio = flip_ratio
        self.direction = direction
        if flip_ratio is not None:
            assert flip_ratio >= 0 and flip_ratio <= 1
        assert direction in ['horizontal', 'vertical']
        self.parsing_map_for_densepose = {
            2:3,
            4:5,
            6:7,
            8:9,
        ...
        if dataset == 'mhp':
            self.parsing_map = self.parsing_map_for_mhp
        elif dataset == 'densepose':
            self.parsing_map = self.parsing_map_for_densepose
        elif dataset == 'chip':
            self.parsing_map = self.parsing_map_for_chip

        print('random flip for ' + dataset + ' dataset')
    ...
    def keypoints_flip(self, keypoints, img_shape, direction):
        ...
        if direction == 'horizontal':
            w = img_shape[1]
            inds = keypoints[...,0]>0
            flipped[..., 0][inds] = w - keypoints[..., 0][inds] - 1
            for p_k,p_v in self.keypoints_map.items():
                tmp = flipped[:,p_k,:].copy()
                flipped[:,p_k,:] = flipped[:,p_v,:]
                flipped[:,p_v,:] = tmp
        elif direction == 'vertical':
            h = img_shape[0]
            flipped[..., 1::4] = h - keypoints[..., 3::4] - 1
            flipped[..., 3::4] = h - keypoints[..., 1::4] - 1
        else:
            raise ValueError(
                'Invalid flipping direction "{}"'.format(direction))
        return flipped
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('parsing_fields', []):
                ...
                for parsing in results[key]:
                    for p_k, p_v in self.parsing_map.items():
                        ind_k = parsing == p_k
                        ind_v = parsing == p_v
                        parsing[ind_v] = p_k
                        parsing[ind_k] = p_v
...
class Albu(object):
    ...
    def __call__(self, results):
        ...
        if 'gt_parsing' in results:
            ...
            # random_params = self.aug[0].get_params()
            # #results['gt_bboxes'] = [x for x in results['gt_bboxes']]
            # results['img'] = self.aug[0].apply(results['img'], **random_params)
            # #random_params_bboxes = dict(rows=results['gt_parsing'][0].shape[0],cols=results['gt_parsing'][0].shape[1])
            # #results['gt_bboxes'] = self.aug[0].apply_to_bbox(results['gt_bboxes'], random_params['angle'], random_params['scale'], random_params['dx'], random_params['dy'], **random_params_bboxes)
            # gt_parsing = []
            # for parsing in results['gt_parsing']:
            #     gt_parsing.append(self.aug[0].apply_to_mask(parsing, **random_params))
            # results['gt_parsing'] = gt_parsing
            # gt_masks = []
            # for mask in results['gt_masks']:
            #     gt_masks.append(self.aug[0].apply_to_mask(mask, **random_params))
            # results['gt_masks'] = gt_masks

            for img_aug in self.img_augs:
                results['img'] = img_aug.apply(results['img'])
            #import pdb;pdb.set_trace()
            results['img'] = results['img']
            
            return results
            #transformed = self.aug(image=results['img'], bboxes=results['gt_bboxes'] , masks=results['gt_parsing'])
        else:
