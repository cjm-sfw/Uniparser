...
class CHIP(Dataset):
    ...
    CLASSES = None
    PARSING_CLASSES = ('Hat', 'Hair', 'Glove', 'Sunglasses', 'Upper-clothes', 
    ...
    def __init__(self,
        ...
        infos = json.load(open(data_root + "CHIP_{}.json".format(self._set), 'r'))
        self.img_infos = infos['images']
        self.ann_infos = infos['annotations']
        
        self.img_infos = self.dict2list(self.dict_generate(self.img_infos, self.ann_infos))
        self.ann_infos = self.img_infos
    ...
    def dict_generate(self, img, ann):
        datas = {}
        for i,ann_info in enumerate(ann):
            img_id = ann_info['image_id']
            bbox = ann_info['bbox']
            if img_id not in datas.keys():
                datas[img_id] = {}
                datas[img_id]['filepath'] = self.img_prefix + img[img_id-1]['file_name']
                datas[img_id]['width'] = img[img_id-1]['width']
                datas[img_id]['height'] = img[img_id-1]['height']
                datas[img_id]['bboxes'] = []
            datas[img_id]['bboxes'].append({
                        'class': 'person',
                        'ann_path': self.ann_file + ann_info['parsing'],
                        'x1': bbox[0],
                        'y1': bbox[1],
                        'x2': bbox[0] + bbox[2],
                        'y2': bbox[1] + bbox[3],
    ...
    def dict2list(self, ann):
        result = []
        for i,ann_info in enumerate(ann):
            result.append(ann[ann_info])
        return result
